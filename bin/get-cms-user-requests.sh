...
if [ -f $CONFIG ]; then
  ...
  CONDOR_POOL=$(awk -F "=" '/^condor_pool/ {print $2}' $CONFIG | tr -d ' ')
  ...
  CONDOR_POOL="vocms099.cern.ch,vocms0115.cern.ch"
...
(
  ...
  /usr/libexec/htcondor-pilot-job-router/get_user_requests.py --pool $CONDOR_POOL 2>/dev/null 1>${USER_REQUESTS_JSON}.new
  mv -f ${USER_REQUESTS_JSON}.new $USER_REQUESTS_JSON
